...
impl BTree {
    ...
    pub fn open(
        ...
        // Verify that the root page exists and is valid
        {
            ...
            match page {
                Page::BTree(btree_page) => match (tree_type, btree_page.header.page_type) {
                    (TreeType::Table, PageType::TableLeaf)
    ...
    fn propagate_split(
        ...
        let cell = if self.tree_type == TreeType::Table {
            BTreeCellFactory::create_table_interior_cell(left_node.page_number, median_key)
        } else {
        ...
        if split {
            ...
            self.propagate_split(parent_node, new_parent.unwrap(), new_median.unwrap(), path)?;
        }
...
mod tests {
    ...
    fn test_create_table_btree() {
        ...
        let mut pager_ref = pager.borrow_mut();
        let page = pager_ref
            .get_page(btree.root_page, Some(PageType::TableLeaf))
            .unwrap();
    ...
    fn test_create_index_btree() {
        ...
        let mut pager_ref = pager.borrow_mut();
        let page = pager_ref
            .get_page(btree.root_page, Some(PageType::IndexLeaf))
            .unwrap();
